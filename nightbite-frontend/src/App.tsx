...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'home' | 'categories' | 'cart' | 'mystery' | 'profile'>('home');
  ...
  const handleAddToCart = (item: CartItem) => {
    const existingIndex = cart.findIndex(
      (cartItem) =>
        ...
        cartItem.selectedSweetness === item.selectedSweetness
    );
    ...
    triggerToast(`🍰 Đã xếp ${item.quantity}x ${item.product.name} vào giỏ hàng`);
  };
  ...
  const loadPromoToCart = () => {
    ...
    const item1: CartItem = {
      ...
      selectedSweetness: '100% Ngọt Chuẩn',
    };
    const item2: CartItem = {
      product: PRODUCTS[2],
      quantity: 2,
      selectedSize: 'Standard Size',
      selectedSweetness: 'Ngọt nhẹ bơ',
    };
    setCart([item1, item2]);
    setActiveTab('cart');
    triggerToast('🛒 Đã tải sẵn đơn bánh kem + sừng bò vào giỏ!');
  };
  ...
  return (
    <div className="w-screen h-screen bg-[#FAF8F5] flex flex-col relative overflow-hidden select-none font-sans">
      ...
      <div className="h-16 bg-white border-t border-[#E8E2D9] grid grid-cols-5 items-center select-none py-1.5 px-1 font-sans shrink-0">
        {/* Trang chủ tab */}
        <button
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 transition-all ${
            activeTab === 'categories' ? getThemeTextClass() : 'text-stone-400 hover:text-stone-600'
          }`}
        >
          <Grid size={15} className={activeTab === 'categories' ? 'stroke-[2.5]' : 'stroke-1.5'} />
          <span className={`text-[9px] font-bold ${activeTab === 'categories' ? 'font-black' : 'font-medium'}`}>Danh mục</span>
        </button>
        ...
        >
          <div className="relative">
            ...
            {cart.length > 0 && (
              <span className="absolute -top-1.5 -right-2 bg-rose-500 text-white font-sans text-[8.5px] font-black h-4 w-4 rounded-full flex items-center justify-center border border-white">
                {cart.reduce((sum, i) => sum + i.quantity, 0)}
              </span>
            )}
          </div>
          <span className={`text-[9px] font-bold ${activeTab === 'cart' ? 'font-black' : 'font-medium'}`}>Giỏ hàng</span>
        </button>
